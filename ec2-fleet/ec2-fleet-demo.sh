...
aws ec2 create-fleet --cli-input-json file://ec2-fleet-template-instance.json
echo "End time for ec2-fleet=$(date)"

cat ec2-fleet-no-errors.txt  | jq -r '.Instances[0].InstanceIds'

cat ec2-fleet-no-errors.txt  | jq -r '.Instances[0].Lifecycle'

cat ec2-fleet-no-errors.txt  | jq -r '.Instances[0].FleetId'
cat ec2-fleet-no-errors.txt  | jq -r '.FleetId'

cat ec2-fleet-no-errors.txt  | jq -r '.Errors'
...
aws ec2 describe-fleets --fleet-id fleet-badc6577-a177-075b-069a-810ae31543ca

aws ec2 delete-fleets --fleet-id fleet-92dc4f5d-23f5-0dd3-8c10-8b888f2fe236 --terminate-instances

aws ec2 delete-fleets --fleet-id fleet-badc6577-a177-075b-069a-810ae31543ca --terminate-instances
